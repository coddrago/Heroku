...
class Translations(loader.Module):
    ...
    def _get_non_downloaded_langs(self) -> typing.Optional[str]:
        langs = self._db.get(translations.__name__, "lang", None)
        if not langs:
            return None
        result = " ".join(lang for lang in langs.split() if not utils.check_url(lang))
    ...
    async def _delete_downloaded(self, call: InlineCall, url: str):
        downloaded = [lang for lang in self._get_downloaded_langs() if lang != url]
        non_downloaded = self._get_non_downloaded_langs()
